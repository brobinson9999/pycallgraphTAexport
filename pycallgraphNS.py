...
# namespace to group functions from the main program (i.e. the one that calls pycallgraphNS)
originNamespace = "PROGRAM"
...
def reset_trace():
    ...
    global call_stack
    global func_count
    global func_count_between
    global func_count_max
    global func_time
    global func_time_max
    global call_stack_timer
    global namespace_time
    global namespace_stack
    global namespace_time_dictionary
    ...
    call_stack_timer = []
    namespace_stack = ['PROGRAM']
    namespace_time_dictionary = {}
...
def start_trace(reset=True, filter_func=None, time_filter_func=None):
    ...
    namespace_time_dictionary['PROGRAM'] = time.time()
...
def stop_trace():
    ...
    # Wind down the namespace stack.
    while (len(namespace_stack) > 0 and namespace_stack[len(namespace_stack)-1] <> ""):
        returnedFromNamespace(namespace_stack[len(namespace_stack)-1])
...
def tracer(frame, event, arg):
    ...
    if event == 'call':
        ...
        if keep:
            ...
            if (len(call_stack) > 0):
                func_call_between = "%s->%s" % (call_stack[len(call_stack)-1], full_name)            
            else:
                func_call_between = "%s->%s" % ("__main__", full_name)
                
            if full_name not in func_count:
                func_count[full_name] = 0
            if func_call_between not in func_count_between:
                func_count_between[func_call_between] = 0
            func_count[full_name] += 1
            func_count_between[func_call_between] += 1
        ...
        #print "Entering: %s" % full_name
        enteredNamespace(getContainingNamespace(full_name))
...
#            if time_filter(stack=call_stack, full_name=full_name):
            #print "Returned from: %s" % full_name
            returnedFromNamespace(getContainingNamespace(full_name));
            
    return tracer

# Every call,
#   If namespace not in namespace_stack, set dictionary time
#   Push namespace regardless
def enteredNamespace(namespace_name):
    if not (namespace_name in namespace_stack):
        namespace_time_dictionary[namespace_name] = time.time();
    namespace_stack.append(namespace_name);

    #print "entered %s" % (namespace_name)
    #print "stack %s" % namespace_stack
 
    containingNamespace = getContainingNamespace(namespace_name)
    if (containingNamespace <> "" and containingNamespace <> originNamespace):
        enteredNamespace(containingNamespace)
        
# Every return,
#   pop namespace
#   if namespace not in stack
#     get push time from dictionary
#     add difference between push time and current time to running total
def returnedFromNamespace(namespace_name):
    t = None
    if namespace_stack:
#        print "popped %s: %s %s" % (namespace_stack.pop(), (namespace_name in namespace_stack), len(namespace_stack))
        namespace_stack.pop()
        if not (namespace_name in namespace_stack):
            t = namespace_time_dictionary[namespace_name]
    if t:
        if namespace_name not in namespace_time:
            namespace_time[namespace_name] = 0
        call_time = (time.time() - t)
        #print "adding time to namespace %s: %s" % (namespace_name, call_time)
        namespace_time[namespace_name] += call_time
    
    #print "returned from %s" % (namespace_name)
    #print "stack %s" % namespace_stack

    containingNamespace = getContainingNamespace(namespace_name)
    if (containingNamespace <> "" and containingNamespace <> originNamespace):
        returnedFromNamespace(containingNamespace)
